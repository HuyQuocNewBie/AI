...
def a_star_search(last_word, used, turn, tu_map, depth_limit=3):  # Giảm độ sâu tìm kiếm
    ...
    while not open_set.empty():
        ...
        # Giới hạn số từ AI xem xét (ví dụ: chỉ chọn 3-5 từ đầu tiên)
        random.shuffle(valid_moves)  # Trộn danh sách từ hợp lệ
        valid_moves = valid_moves[:min(3, len(valid_moves))]  # Chỉ lấy tối đa 3 từ

        # Nếu đến lượt của đối thủ mà không còn nước đi nào, AI thắng
        if current_turn == "player" and len(valid_moves) == 0:
            return True, seq  # AI thắng

        # Nếu đến lượt của AI mà không còn nước đi, AI thua
    ...
    return False, []  # AI không tìm được nước đi
